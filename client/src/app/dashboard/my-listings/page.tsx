...
export default function MyListingsPage() {
  ...
  const [searchInput, setSearchInput] = useState(''); // Local input state (no re-render)
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setSearchTerm(searchInput); // Update search term after delay
    }, 200); // Réduit à 200ms
  ...
  const fetchListings = async () => {
    try {
      ...
      // ✅ FIX BQ-8, BQ-43: Extract pagination info with better fallback logic
      const paginationData = response.data.pagination || response.data.meta;
      if (paginationData) {
        const pages = paginationData.pages || paginationData.totalPages || Math.ceil((paginationData.total || 0) / itemsPerPage);
                  ...
                  <tr key={listing._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <p className="text-xs text-gray-500">
                        {/* ✅ FIX BQ-3: Display correct pricing unit based on pricingType */}
                        {listing.pricing.pricingType === 'per_day' ? 'Par Jour' :
                         listing.pricing.pricingType === 'per_night' ? 'Par Nuit' :
                         listing.pricing.pricingType === 'per_hour' ? 'Par Heure' :
                         listing.pricing.pricingType === 'per_week' ? 'Par Semaine' :
                         listing.pricing.pricingType === 'per_month' ? 'Par Mois' :
                         (listing.category === 'vehicle' ? 'Par Jour' : 'Par Nuit')}
                      </p>
                    </td>
                    <td className="px-6 py-4">
